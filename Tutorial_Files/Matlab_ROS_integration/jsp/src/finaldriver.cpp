...
void finaldriverModelClass::step()
{
  ...
  if (rtmIsMajorTimeStep((&finaldriver_M))) {
    ...
    if (!((&finaldriver_M)->Timing.clockTick0+1)) {
      rtsiSetSolverStopTime(&(&finaldriver_M)->solverInfo, (((&finaldriver_M)
        ->Timing.clockTickH0 + 1) * (&finaldriver_M)->Timing.stepSize0 *
        4294967296.0));
    } else {
      rtsiSetSolverStopTime(&(&finaldriver_M)->solverInfo, (((&finaldriver_M)
        ->Timing.clockTick0 + 1) * (&finaldriver_M)->Timing.stepSize0 +
        (&finaldriver_M)->Timing.clockTickH0 * (&finaldriver_M)
  ...
  finaldriver_B.FilterCoefficient = (5.0 * rtb_Sum1 -
    finaldriver_X.Filter_CSTATE) * 100.0;
  ...
  if (rtmIsMajorTimeStep((&finaldriver_M))) {
     ...
     *  Sum: '<S4>/FixPt Sum1'
     */
    rtb_FixPtSwitch = static_cast<uint8_T>(static_cast<uint8_T>
      (finaldriver_B.Output + 1U) & 31);
  }
...
void finaldriverModelClass::initialize()
{
  /* Registration code */
  {
    ...
    rtsiSetContStatesPtr(&(&finaldriver_M)->solverInfo, (real_T **)
                         &(&finaldriver_M)->contStates);
    ...
    rtsiSetErrorStatusPtr(&(&finaldriver_M)->solverInfo, (&rtmGetErrorStatus
      ((&finaldriver_M))));
  ...
  rtsiSetSolverData(&(&finaldriver_M)->solverInfo, static_cast<void *>
                    (&(&finaldriver_M)->intgData));
